<template>
  <div class="cart">
    <RouterLink class="curr" to="/cart">
      ...
      ><em>{{ $store.state.cart.goodsList.length }}</em>
    </RouterLink>
    <div class="layer" v-if="effectiveGoodsCount > 0 && !isCartPage">
      <div class="list">
        <div
          class="item"
          v-for="goods in $store.state.cart.goodsList"
          :key="goods.id"
        >
          <RouterLink :to="`/goods/detail/${goods.id}`">
            ...
            <div class="center">
              ...
              <p class="attr ellipsis">{{ goods.arrtsText }}</p>
            </div>
            <div class="right">
          ...
          <i class="iconfont icon-close-new" @click="removeCartGoods(goods.skuId)"></i>
        </div>
      </div>
      <div class="foot">
        <div class="total">
...
export default defineComponent({
  ...
  setup() {
    ...
    // 计算可购买商品列表的getters (有效商品，并且库存数量大于0)
    ...
    let isCartPage = ref(route.path === '/cart');
